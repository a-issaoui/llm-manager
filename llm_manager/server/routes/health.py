...
from ..dependencies import get_llm_manager, _server_config, verify_api_key
...
async def health_check_openai(manager: LLMManager = Depends(get_llm_manager)):
...
@router.get("/ready")
async def readiness_check(manager: LLMManager = Depends(get_llm_manager)):
...
async def server_stats(
    ...
    if manager.is_loaded() and manager.model_path:
        stats["current_model"] = {
            "name": manager.model_path.stem,
            "path": str(manager.model_path)
        }
