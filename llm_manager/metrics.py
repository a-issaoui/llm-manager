...
class MetricsCollector:
    ...
    def record_callback(
        ...
        class MetricsContext:
            def __init__(ctx_self, collector, model, tokens):
            ...
            def set_output_tokens(ctx_self, tokens: int):
                """Set output token count."""
                ctx_self.output_tokens = tokens
        
        return MetricsContext(self, model_name, input_tokens)
    
    def get_stats(self, window_seconds: Optional[float] = None) -> PerformanceStats:
        """
    ...
    def get_model_breakdown(self) -> Dict[str, Dict[str, Any]]:
        ...
        for model, requests in by_model.items():
            ...
            breakdown[model] = {
                ...
                "tokens_per_second": round(total_output / total_duration, 2) if total_duration > 0 else 0,
