...
from llm_manager.server import LLMServer, create_app
...
class TestServerDependencies:
    ...
    def test_configure_server(self):
        ...
        configure_server(models_dir="/test/models", api_key="test-key", default_model="test-model")
...
class TestGetOrLoadModel:
    ...
    async def test_model_already_loaded(self):
        ...
        manager = Mock()
        manager.get_or_load_async = AsyncMock(return_value=True)

        result = await get_or_load_model(manager, "test-model")
        assert result is manager
        manager.get_or_load_async.assert_called_once_with("test-model")
    ...
    async def test_load_new_model_success(self):
        ...
        assert result is manager
        manager.get_or_load_async.assert_called_once_with("new-model.gguf")
    ...
    async def test_model_not_found(self):
        ...
        manager = Mock()
        manager.get_or_load_async = AsyncMock(side_effect=ModelNotFoundError("Model not found"))
    ...
    async def test_ambiguous_model_name(self):
        ...
        manager = Mock()
        manager.get_or_load_async = AsyncMock(side_effect=ValueError("Ambiguous"))
    ...
    async def test_load_model_failure(self):
        ...
        from llm_manager.exceptions import ModelLoadError

        manager = Mock()
        manager.get_or_load_async = AsyncMock(side_effect=ModelLoadError("Load failed"))
...
class TestServerInitialization:
    ...
    def test_llm_server_class(self, tmp_path):
        ...
        server = LLMServer(models_dir=str(tmp_path), port=9000, host="0.0.0.0", api_key="test")
...
class TestCompletionEndpointExtended:
    ...
    def test_completion_with_suffix(self, tmp_path):
        ...
        mock_response = {"choices": [{"message": {"content": "middle"}}]}
        manager.generate_async = AsyncMock(return_value=mock_response)
    ...
    def test_completion_with_echo(self, tmp_path):
        ...
        mock_response = {"choices": [{"message": {"content": " completion"}}]}
        manager.generate_async = AsyncMock(return_value=mock_response)
        ...
        with patch.object(comp_module, "get_or_load_model", mock_get_or_load):
            ...
            response = client.post(
                "/v1/completions",
                json={"model": "test-model", "prompt": "Prefix", "echo": True, "max_tokens": 50},
            )
