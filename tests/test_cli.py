...
class TestCLIMainExecution:
    ...
    def test_main_creates_server(self, mock_isdir, mock_server_class):
        ...
        assert "models_dir" in call_kwargs or any(
            "./models" in str(v) for v in call_kwargs.values()
        )
    ...
    def test_main_with_default_model(self, mock_isdir, mock_server_class):
        ...
        with patch(
            "sys.argv", ["llm-manager", "--models-dir", "./models", "--model", "qwen2.5-7b"]
        ):
            try:
                cli.main()
            except SystemExit:
...
class TestCLIExamples:
    ...
    def test_example_with_model(self, mock_isdir, mock_server_class):
        ...
        with patch(
            "sys.argv", ["llm-manager-cli", "--models-dir", "./models", "--model", "qwen2.5-7b"]
        ):
            try:
                cli.main()
            except SystemExit:
