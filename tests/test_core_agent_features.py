        ...
        with patch.object(manager, "unload_model") as mock_unload:
            with patch.object(manager, "load_model", return_value=True) as mock_load:
                ...
                mock_load.assert_called_once_with(
                    model_path="model1.gguf", n_ctx=None, n_gpu_layers=-1, auto_context=True
                )
        ...
        with patch.object(manager, "unload_model"):
            with patch.object(manager, "load_model", return_value=True) as mock_load:
                manager.switch_model(
                    "model.gguf", n_ctx=8192, n_gpu_layers=20, auto_context=False, flash_attn=True
                )
...
class TestGenerateVariations:
    ...
    async def test_generate_variations_basic(self, tmp_path):
        ...
        expected_responses = [
            {"choices": [{"message": {"content": f"Variation {i}"}}]} for i in range(3)
        ]
...
class TestAgentFeaturesIntegration:
    ...
    async def test_agent_workflow(self, tmp_path):
        ...
        with patch.object(
            manager,
            "generate_async",
            return_value={"choices": [{"message": {"content": "Initial response"}}]},
        ):
        ...
        with patch.object(
            manager,
            "generate_async",
            side_effect=[{"choices": [{"message": {"content": f"V{i}"}}]} for i in range(3)],
        ):
            variations = await manager.generate_variations(
                [{"role": "user", "content": "Solve"}], n_variations=3
            )
            assert len(variations) == 3
